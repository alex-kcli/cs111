...
//    clock_gettime(CLOCK_REALTIME, &ts);
    ...
    SSL_write(ssl_client, temp_buf, strlen(temp_buf));
...
void do_when_pushed() {
    ...
    char *shutdown_buf = "";
    sprintf(shutdown_buf, "%02d:%02d:%02d SHUTDOWN\n", tm->tm_hour, tm->tm_min, tm->tm_sec);
    SSL_write(ssl_client, shutdown_buf, strlen(shutdown_buf));
...
int main(int argc, char * argv[])
{
    ...
    /* initialize ssl transfer */
    context = ssl_init(); // newContext, init library, error string, algorithms
    ssl_client = attach_ssl_to_socket(sckt, context); //SSL_new(), SSL_set_fd(),SSL_connect()
    ...
    while(1) {
        ...
        if (ret < 0) {
            ...
            int readFile = SSL_read(ssl_client, buffer, 128);
            if (readFile < 0) {
